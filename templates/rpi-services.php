<?php
/**
 * Created by PhpStorm.
 * User: Joachim
 * Date: 01.02.2017
 * Time: 01:42
 * <i class="fa fa-newspaper-o"></i> <i class="fa fa-search"></i><i class="fa fa-users"></i><i class="fa fa-pencil-square-o"></i>
 */

?>
<div id="more-rpi-container-sidebar" title="Dienste von rpi-virtuell">
    <div class="rpi-container-sidebar-wrapper">
		<a id="rpi-container-sidebar-button" href="#rpi-services">Dienste</a>
		<div class="rpi-container-sidebar-content">
			<p class="rpi-container-sidebar-title">Kerndienste von rpi-virtuell</p>
			<div class="rpi-core-services">
				<ul>
					<li class="rpi-service-news">
						<a href="https://news.rpi-virtuell.de/">
							<span class="service-label">
								news
							</span><br>
							Bildungsnachrichten
						</a>
					</li>
					<li class="rpi-service-materialpool">
						<a href="https://material.rpi-virtuell.de/">
							<span class="service-label">
								material
							</span><br>
							Freie Bildungsmedien
						</a>
					</li>
					<li class="rpi-service-gruppen">
						<a href="https://gruppen.rpi-virtuell.de/">
							<span class="service-label">
								gruppen
							</span><br>
							Soziales Netzwerk
						</a>
					</li>
					<li class="rpi-service-blogs">
						<a href="https://blogs.rpi-virtuell.de/">
							<span class="service-label">
								blogs
							</span><br>
							Eigene Webseiten
						</a>
					</li>
				</ul>
			</div>
			<div class="clear"></div>
			<p class="rpi-container-sidebar-title">Noch mehr von rpi-virtuell</p>
			<div class="rpi-more-services rpi-service-more">
				<ul>
					<li>
						<a href="https://blogs.rpi-virtuell.de/artothek">
							<i class="fa fa-picture-o"></i>
							<span class="service-label">
							Artothek (Kunst-Ausstellungen)
						</span>
						</a>
					</li>
					<li>
						<a href="https://kurse.rpi-virtuell.de/">
							<i class="fa fa-university"></i>
							<span class="service-label">
							Kurse (Lernmanagementsystem)
						</span>
						</a>
					</li>
					<li>
						<a href="https://relilex.de/">
							<i class="fa fa-book"></i>
							<span class="service-label">
							relilex (Lexikon Religion)
						</span>
						</a>
					</li>
					<li>
						<a href="https://openreli.de/">
							<i class="fa fa-graduation-cap"></i>
							<span class="service-label">
							openreli (offener Online Kurs)
						</span>
						</a>
					</li>
				</ul>
			</div>
			<div class="rpi-about-link">
				<ul>
					<li>
						<a href="https://rpi-virtuell.de/">
							<i class="fa fa-info-circle"></i>
							<span class="service-label">
							Über rpi-virtuell
						</span>
						</a>
					</li>
					<li>
						<a href="https://hilfe.rpi-virtuell.de/support">
							<i class="fa fa-medkit"></i>
							<span class="service-label">
							Support (Wir helfen gerne!)
						</span>
						</a>
					</li>
					<li>
						<a href="https://hilfe.rpi-virtuell.de/">
							<i class="fa fa-question-circle"></i>
							<span class="service-label">
							Fragen und Antworten
						</span>
						</a>
					</li>
				</ul>

			</div>
		</div>
    </div>
</div>
